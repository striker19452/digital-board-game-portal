...
三个电子桌游的统一入口，支持公网、本机和局域网两套启动环境。游戏保持独立部署，门户不读取游戏存档或房间数据。
...
新增游戏可以继续使用 `4177`、`4178` 等未分配端口。清单校验会检查游戏之间是否重复；实际启动时还会检查端口是否已被其他程序占用。
...
- 怪兽猎人和镇夜录已在清单中配置预期的 GitHub Pages 地址。
